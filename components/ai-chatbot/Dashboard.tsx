...
function ChartCard({ title, caption, children }: { title: string; caption?: string; children: React.ReactNode }) {
  return (
    <div className="bg-[#242424] rounded-2xl p-6 border border-white/5">
      <p className="text-xs font-semibold tracking-widest text-gray-500 uppercase mb-1">{title}</p>
...
export default function Dashboard({ kpi, trendData, retentionData, returnData }: Props) {
  return (
    <div className="min-h-screen bg-[#1A1A1A] p-8">
      {/* Header */}
      <div className="mb-8">
        <p className="text-xs font-semibold tracking-widest text-gray-500 uppercase mb-1">Experiment Report</p>
        <h1 className="text-2xl font-bold text-white">AI Chatbot A/B Test</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <KpiCard
          ...
          value={`${kpi.testEsc}%`}
          delta={kpi.testEsc - kpi.ctrlEsc}
          downIsGood
        />
        <KpiCard
          label="Ticket Creation (O2C)"
          value={`${kpi.testO2c}%`}
          delta={kpi.testO2c - kpi.ctrlO2c}
          downIsGood
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <ChartCard
          ...
          caption="% of users escalated to a human agent · Other Issues bucket"
        >
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={trendData} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
              ...
              <YAxis tickFormatter={(v) => `${Math.round(v)}%`} tick={{ fontSize: 11, fill: "#525252" }} axisLine={false} tickLine={false} />
        ...
        <ChartCard
          title="Ticket Creation Rate (O2C)"
          caption="Tickets per support interaction · Other Issues bucket"
        >
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={trendData} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#2E2E2E" />
        ...
        <ChartCard title="Retention Rate" caption="Returned to use support">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={retentionData} margin={{ top: 4, right: 8, left: -16, bottom: 0 }} barGap={4}>
              ...
              <XAxis dataKey="window" tick={{ fontSize: 11, fill: "#525252" }} axisLine={false} tickLine={false} />
              <YAxis tickFormatter={(v) => `${v}%`} tick={{ fontSize: 11, fill: "#525252" }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={tooltipStyle} formatter={(v) => [`${Number(v).toFixed(1)}%`]} />
              <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11, paddingTop: 12 }} />
